...
class RQAI:
    ...
    def __init__(self):
        ...
        if __name__ == "RQAI": # Check to ensure that only the RQAI thread can make a daemon as a new process. This will always evaluate as true.
            daemon = multiprocessing.Process(target=RQAIDaemon, args=(q,)) # Creates the daemon as a new process.
            daemon.daemon = True # Sets the process ID to be that of a daemon    
...
class RQAIDaemon:
    ...
    def __init__(self, cDirectory, connectionQ, bDirectory, bFileLog):
        ...
        for filename in os.listdir(self.cDirectory): # Loads the files in the cDirectory and commits them to local memory.               
            ...
            self.connectionQ.put(f"Loading file ({currentFile}/{count})") # UI information
            self.VerifyFileIntegrity(filename) # Reading the RQAI Files and committing them to local memory
        ...
        for item in os.listdir(self.cDirectory): # For every item in the cDirectory
            ...
            try: 
                if (contents[0].split(':')[2] == "verified"): # Attempts to read a spot that may not exist
                    backupItem = contents[0].split(':')[3] # Gets the verification date
                    folderName = ".".join(backupItem.split('.')[0], backupItem.split('.')[1], backupItem.split('.')[2]) # Constructs folder name from verification info
                    if not (os.path.isdir(os.path.join(self.bDirectory, folderName))): # If the folder for the day doesn't already exist:
                        os.mkdir(self.bDirectory, folderName) # Make the folder.
                    if not os.path.exists(os.path.join(self.bDirectory, folderName, f"{item}.{backupItem}")): # If the backup item doesn't already exist
                        open(os.path.join(self.bDirectory, folderName, f"{item}.{backupItem}", "x")) # Create the backup item
                        with open(os.path.join(self.bDirectory, f"{item}.{backupItem}", "w")) as newFile: # Open the newly created backup item
                            newFile.writelines(contents) # Write the old file contents to a new file. TODO: Copy file instead of copying contents
                        with open(os.path.join(self.bDirectory, self.bFileLog)) as log: # Opens the system log file
                            contents = log.readlines() # Copies the log files into a local array
                        if (contents.__contains__(folderName)): # Searches through the array to find if the folder exists already:
                            indexOf = contents.index(folderName) # Folder already exists, grabs its index.
                            if not (indexOf + 1 >= len(contents)): # Make sure that we aren't trying to add data to a spot that doesn't exist
                                contents.insert(indexOf + 1, f"{item}:{backupItem}:{os.path.getsize(newFile):,} bytes") # Insert new item
                            else: # We're at the end of the list
                                contents.append(f"{item}:{backupItem}:{os.path.getsize(newFile):,} bytes") # Appends rather than inserts the new value
                        else: # The folder entry doesn't already exist
                            contents.append(folderName) # adds the folder name to the file
                            contents.append(f"{item}:{backupItem}:{os.path.getsize(newFile):,} bytes") # Appends the entry onto the file
