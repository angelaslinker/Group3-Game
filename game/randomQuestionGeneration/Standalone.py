...
import subprocess
import sys # Get CLI (Command Line Input)
...
from RQAI import RQAIDaemon

"""class RandomQuestions:
    def __init__(self):"""
if __name__ == "__main__": # Check to ensure that only the main thread can make a daemon as a new process.
    _RQAI = None
    arguments = sys.argv
    if(arguments.__contains__("-h")):
        ...
        _RQAI = RQAI()
    daemon = multiprocessing.Process(target=RQAIDaemon, args=(_RQAI.cDirectory, _RQAI.connectionQ, _RQAI.bDirectory, _RQAI.bFileLog,)) # Creates the daemon as a new process.
